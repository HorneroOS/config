...
One icon family across both flagships, both variants shipped by a single
official package, no new dependency.

## Evidence

| Candidate | Coverage | License | Packaging | Fit | Verdict |
|---|---|---|---|---|---|
| **Papirus / Papirus-Dark / Papirus-Light** | Largest of the set; actively maintained; per-app coverage far exceeds Numix-Circle | GPL-3.0 | `extra/papirus-icon-theme` (official, Felix Yan). One package ships all three variants (verified via `pacman -Fl`: `Papirus`, `Papirus-Dark`, `Papirus-Light` index.theme files) | Neutral rounded style; recolors cleanly against terracotta/sunset; Dark variant matches the night-adobe ramp | **ADOPT** (both flagships) |
