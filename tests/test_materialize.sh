...
# theme packs: exactly the 14 curated packs (12 recipes + 2 flagship token themes) in canonical hornero/* (rows 1+8)
count=$(find "$TMP_HOME/.local/share/hornero/themes" -maxdepth 2 -name theme.json | wc -l)
if [[ $count -eq 14 ]]; then
  echo "TEST-PASS: 14 theme packs materialized to hornero/themes"
else
  echo "TEST-FAIL: expected 14 theme packs in hornero/themes, found $count" >&2
  exit 1
fi
...
# dots fallback still resolves the 14 packs through the symlink
compat_count=$(find -L "$TMP_HOME/.local/share/dots/themes" -maxdepth 2 -name theme.json | wc -l)
if [[ $compat_count -eq 14 ]]; then
  echo "TEST-PASS: dots/themes fallback resolves 14 packs"
else
  echo "TEST-FAIL: dots fallback resolves $compat_count packs, want 14" >&2
  exit 1
fi
